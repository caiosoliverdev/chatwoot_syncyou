...
export default {
  ...
  methods: {
    ...
    setLocale(locale) {
      this.$root.$i18n.locale = locale || 'pt';
    },
  },
};
</script>
...
<template>
  <div v-if="isLoading" class="flex items-center justify-center flex-1 h-full min-h-screen bg-black-25">
    <Spinner size="" />
  </div>
  <div v-else class="flex items-center justify-center w-full h-full min-h-screen overflow-auto bg-slate-50">
    <div class="flex flex-col w-full h-full bg-white rounded-lg shadow-lg lg:w-2/5 lg:h-auto">
      <div class="w-full px-12 pt-12 pb-6 m-auto my-0">
        <img v-if="logo" :src="logo" alt="SyncYou logo" class="mb-6 logo" />
        <p v-if="!isRatingSubmitted" class="mb-8 text-lg leading-relaxed text-black-700">
        ...
        <Banner v-if="shouldShowBanner" :show-success="shouldShowSuccessMesage" :show-error="shouldShowErrorMesage"
          :message="message" />
        <label v-if="!isRatingSubmitted" class="mb-4 text-base font-medium text-black-800">
          {{ $t('SURVEY.RATING.LABEL') }}
        </label>
        <Rating :selected-rating="selectedRating" @select-rating="selectRating" />
        <Feedback v-if="enableFeedbackForm" :is-updating="isUpdating" :is-button-disabled="isButtonDisabled"
          :selected-rating="selectedRating" @send-feedback="sendFeedback" />
      </div>
      <div class="mb-3">
        <Branding />
      </div>
