...
RSpec.describe 'Api::V1::Accounts::Articles', type: :request do
  ...
  let!(:category) { create(:category, name: 'category', portal: portal, account_id: account.id, locale: 'pt_BR', slug: 'category_slug') }
  ...
  describe 'GET /api/v1/accounts/{account.id}/portals/{portal.slug}/articles' do
    ...
    context 'when it is an authenticated user' do
      ...
      it 'get all articles with uncategorized articles' do
        article2 = create(:article, account_id: account.id, portal: portal, category: nil, locale: 'pt_BR', author_id: agent.id)
        expect(article2.id).not_to be_nil
