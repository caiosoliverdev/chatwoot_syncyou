...
describe Conversations::FilterService do
  ...
  let!(:en_conversation_1) do
    create(:conversation, account: account, inbox: inbox, assignee: user_1, campaign_id: campaign_1.id,
                          status: 'pending', additional_attributes: { 'browser_language': 'pt' })
  end
  ...
  describe '#perform' do
    context 'with query present' do
      ...
      it 'filter conversations by additional_attributes and status' do
        ...
        conversations = Conversation.where("additional_attributes ->> 'browser_language' IN (?) AND status IN (?)", ['pt'], [1, 2])
  ...
  describe '#perform on custom attribute' do
    context 'with query present' do
      ...
      it 'filter by custom_attributes and additional_attributes' do
        ...
        conversations[0].update!(additional_attributes: { 'browser_language': 'pt' }, custom_attributes: { conversation_type: 'silver' })
        conversations[1].update!(additional_attributes: { 'browser_language': 'pt' }, custom_attributes: { conversation_type: 'platinum' })
